...
一个基于 google v8 JavaScript 引擎的非阻塞，事件驱动的平台.
...
* 每个phase都只是一个队列，不是异步任务本身
* 异步任务在执行完成之后会把回调函数推入到队列中，事件循环会把队列中的回调加入到调用栈中（也就是执行）。

#### 为啥从 timer 开始呢？

* 主任务就是从 timer 开始校验的，后期的I/O操作才会从 poll 开始校验。

  因为主任务就相当于一个 I/O 开始（从poll进入）
...
* timer 这个阶段主要执行 settimeout、setinterval 的回调。libuv 的实现是根据最小堆的方式，只要顶部的元素没有超时，后面的节点就不会超时。
...
* idle, prepare 仅在系统内部使用
  ...
  和浏览器的 requestIdleCallback 类似。

* **poll**  检索新的 I/O 事件；执行与 I/O 相关的回调。（除了 关闭回调、定时器回调、和setimmediate回调等几乎所有的回调）

  ==非常重要的阶段，node做服务器所有的I/O回调都是在这个阶段执行的！！！==

  也就是浏览器中的事件循环的主任务执行。（比如setTimeout的回调函数）
...
* 参考：https://nodejs.org/en/docs/guides/event-loop-timers-and-nexttick/#setimmediate-vs-settimeout

* 执行顺序

  两个回调的执行顺序受到他们调用的上下文影响。

  1. 在主任务中调用，那么它们执行的先后顺序坑受到进程性能的限制（比如同步任务的耗时等）。也就是调用的先后顺序不一定

  2. 如果在 I/O 周期内，则从 poll 阶段开始，会先进入check阶段，那么调用的顺序就会一直（先执行setImmediate后执行setTimeout）
...
​	node 做服务器都是从i/o回调 poll 阶段开始的，这样也就不会有任何异议（除了做一次性脚本的需求）。
...
> Node.js属于单线程事件循环架构，该事件循环由Libuv的uv_run函数实现，在该函数中执行while循环，然后不断地处理各个阶段（phase）的事件回调。事件循环的处理相当于一个消费者，消费由各种代码产生的任务。Node.js初始化完成后就开始陷入该事件循环中，事件循环的结束也就意味着Node.js的结束。

* nodejs 作为服务端的脚本，是会结束的，事件循环结束那么nodejs也就退出了。它是一个 while 循环，不断处理各个阶段（phase）的回调。

  每个阶段的执行源码可以参考：https://theanarkh.github.io/understand-nodejs/chapter03-%E4%BA%8B%E4%BB%B6%E5%BE%AA%E7%8E%AF/

## timer.unref

参考：https://zhuanlan.zhihu.com/p/117732823

![img](https://pic2.zhimg.com/80/v2-de18fe8131d57fd227aee01a3a5ab2a5_1440w.webp)
...
除了上面上面 libuv 中的队列，另外还有两个有意思的队列，就是“中间对列”，被node处理，但是不属于libuv的一部分，是nodejs的一部分。分别是

* 下一个运转队列（next ticks queue）- 使用 process.nextTick() 函数回调添加
* **其他的微队列（other Microtasks Queue）** - 包含其他的微队列如成功的 Promise 回调

他们的执行顺序也如上。

![img](https://pic2.zhimg.com/v2-6e362132c0fedffe78cdb5ac12a9fb09_b.jpg)

被描绘成红色的中间队列有趣的是，只要一个阶段结束，事件循环将会检查这两个中间阶段是否有要处理的项。如果有，事件循环会立马开始处理它们直到两个队列为空。一旦为空，事件循环就移到下一个阶段，实际上 next tick queue 比 [micro tasks queue](https://www.zhihu.com/search?q=micro tasks queue&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"article"%2C"sourceId"%3A"37427130"}) 有着更高的优先级。

### 运转队列——next ticks queue
...
* Unref 和 clearTimeout 的异同
  * 共同点：都不会hold住当前的process。
  * 不同点：unref 并不会取消回调函数的执行，取消其执行完全是因为程序退出导致的（如果有一个比这个定时器还长的定时器，那么这个回调函数还是会执行）。clearTimeout 是直接取消回调函数。

#### 用处

* 如何优雅的退出程序呢？

  就是使用 timer.unref，如果在规定的30s内有退出程序，则直接退出（settimeout不卡进程）；如果没有退出，则到了定时触发回调函数退出。
...
* Buffer是内存区域，就是一块内存区域。Buffer所开辟的是堆外内存。==不占据js内存==
  ...
  因为不占据 nodejs 运行内存，因此可以用来存储大内容的文件，能够突破nodejs系统内存限制。
...
* `Buffer.from`：从现有的一个字符串或者数组创建出一个 buffer 实例。长度和字符一一对应。
  ...
  每一项都是一个字节（字节表示的位数是固定的8位），如果from的字符需要三个字节表示，则生产出长度为3的buffer。
...
```typescript
// 使用map迭代直接返回一个新的 buffer
console.log(buffer.map(item => item));
// 使用扩展运算符的形式直接返回转换的10进制数据
console.log([...buffer])
```
...
* write方法
  ...
  直接会替换掉buffer的内容，*使用 wite 修改内容，如果字节长度超出，则会截断*
...
* 如果一个字符串已经采用了其他编码方式进行了编码，怎么转换为 utf-8 呢？
...
* 主要是 read**BE & read\*\*LE，BE 和 LE 区别就是大小。大端和小端。https://developer.mozilla.org/zh-CN/docs/Glossary/Endianness
  ...
  获取本机字节序：os.endianness()

* ==只有超过两个字节的才能写大端和小端，比如说16位==

```typescript
// 16位代表需要写前两位元素（也就是两个字节）。81也就是Q，如果需要写入数字，每个字节只能写入一位。
// buffer.writeInt16BE(81)
// LE、BE只是字节序，按照某种字节序写入，不直接关联操作系统。但是建议和操作系统保持一致。
buffer.writeInt16LE(81)
```
...
console.log(
  // 方式1：Buffer.concat
  Buffer.concat([buffer, buffer]).toString(),
  // 方式2：数组的方式合并
  Buffer.from([...buffer, ...buffer]).toString(),
  // buffer copy
  newBuf.toString(),
  // 直接相加
  '' + buffer + buffer,
...
* 也就是提供了一种方式去处理大数据缓存的能力，比如说处理 Buffer（node.js内存之外的缓存）

* 推荐阅读

  https://juejin.cn/post/6844903891083984910#heading-9
  
  https://juejin.cn/post/6934987500540657701#heading-0
...
注意：==不要把stream想象成一个存储结构，它只是一个读取buffer的对象，真正的存储结构是Buffer。==
...
* buffer 是实际存储的实体，stream并不存储，只是为了读取buffer
...
> 而我们通常在进行文件读写时，操作的其实是字节流，所以在设置流参数 `options` 时需要注意编码格式，这是会影响 `chunk` 的内容和大小。而可读流与可写流默认的编码格式并不同，而每种不同的流也都不相同，所以在使用流操作前一定要先看默认参数设定，以免发生数据积压问题。

* 创建流的时候需要注意编码方式，防止出现读取不一的情况或者影响chunk内容和大小的情况

  主要发生在可读流和可写流流速不一致的情况
...
> **可读流和可写流都会在内部的缓冲器中存储数据**，对于非对象流来说，`highWaterMark` 指定了**字节**的总数。实际上 `highWaterMark` 只是一个**阈值**，它并**不会限制写入缓冲的数据大小**，除非直接突破 Node.js 缓冲区最大值。

* highWaterMark 用来控制每次读取的 buffer 长度。也就是内部缓冲区的字节总数，不是一个固定值，只是一个阈值。
...
> [Writable](https://link.juejin.cn?target=http%3A%2F%2Fnodejs.cn%2Fapi%2Fstream.html%23stream_class_stream_writable) - 可写入数据的流（例如 [`fs.createWriteStream()`](https://link.juejin.cn?target=http%3A%2F%2Fnodejs.cn%2Fapi%2Ffs.html%23fs_fs_createwritestream_path_options)让我们以流的形式将数据写入文件）。
>
> [Readable](https://link.juejin.cn?target=http%3A%2F%2Fnodejs.cn%2Fapi%2Fstream.html%23stream_class_stream_readable) - 可读取数据的流（例如 [`fs.createReadStream()`](https://link.juejin.cn?target=http%3A%2F%2Fnodejs.cn%2Fapi%2Ffs.html%23fs_fs_createreadstream_path_options)让我们以流的形式从文件中读取数据）。
>
> [Duplex](https://link.juejin.cn?target=http%3A%2F%2Fnodejs.cn%2Fapi%2Fstream.html%23stream_class_stream_duplex) - 可读又可写的流（例如 [`net.Socket`](https://link.juejin.cn?target=http%3A%2F%2Fnodejs.cn%2Fapi%2Fnet.html%23net_class_net_socket)）。
>
> [Transform](https://link.juejin.cn?target=http%3A%2F%2Fnodejs.cn%2Fapi%2Fstream.html%23stream_class_stream_transform) - 在读写过程中可以修改或转换数据的 Duplex 流，转换流（Transform）也是一种 Duplex 流，但它的输出与输入是相关联的。 与 Duplex 流一样， Transform 流也同时实现了 Readable 和 Writable 接口。（例如[` zlib.createDeflate()`](https://link.juejin.cn?target=http%3A%2F%2Fnodejs.cn%2Fapi%2Fzlib.html%23zlib_zlib_createdeflate_options)）。
>
> 
>
> 作者：字节跳动ADFE团队
> 链接：https://juejin.cn/post/6934987500540657701
> 来源：稀土掘金
> 著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
...
> 在 flowing 模式下， 可读流自动从系统底层读取数据，并通过 [`EventEmitter`](https://www.nodeapp.cn/events.html#events_class_eventemitter) 接口的事件尽快将数据提供给应用。
>
> 在 paused 模式下，必须显式调用 [`stream.read()`](https://www.nodeapp.cn/stream.html#stream_readable_read_size) 方法来从流中读取数据片段。

* Flowing 模式下，可读流自动从系统底层读取数据，并且动过 eventEmitter 接口事件将数据提供给应用。比如 data 事件
* paused 模式下，需要显式的调用 read() 方法来读取数据

默认情况下都是 paused 模式，需要调用一下方法切换到 flowing 模式：

> 所有初始工作模式为 paused 的 [Readable](https://www.nodeapp.cn/stream.html#stream_class_stream_readable) 流，可以通过下面三种途径切换到 flowing 模式：
>
> - 监听 [`'data'`](https://www.nodeapp.cn/stream.html#stream_event_data) 事件。
> - 调用 [`stream.resume()`](https://www.nodeapp.cn/stream.html#stream_readable_resume) 方法。
> - 调用 [`stream.pipe()`](https://www.nodeapp.cn/stream.html#stream_readable_pipe_destination_options) 方法将数据发送到 [Writable](https://www.nodeapp.cn/stream.html#stream_class_stream_writable)。

切换至 paused 模式：

可读流可以通过下面途径切换到 paused 模式：

- 如果不存在管道目标（pipe destination），可以通过调用 [`stream.pause()`](https://www.nodeapp.cn/stream.html#stream_readable_pause) 方法实现。
- 如果存在管道目标，可以通过取消 [`'data'`](https://www.nodeapp.cn/stream.html#stream_event_data) 事件监听，并调用 [`stream.unpipe()`](https://www.nodeapp.cn/stream.html#stream_readable_unpipe_destination) 方法移除所有管道目标来实现。
...
> *注意*: 如果 [Readable](https://www.nodeapp.cn/stream.html#stream_class_stream_readable) 切换到 flowing 模式，且没有消费者处理流中的数据，这些数据将会丢失。 比如， 调用了 `readable.resume()` 方法却没有监听 `'data'` 事件，或是取消了 `'data'` 事件监听，就有可能出现这种情况。

* 切换到 flowing 模式就开始消费，如果消费者没有处理数据，这些消费过的数据将会丢失。
...
* 这个感觉有点缘木求鱼了，违背了js单线程的特性。
* 多年来，Node.js 一直都不是实现 CPU 密集型应用的最佳选择。在CPU密集型计算的应用中，node的表现一直不好，这也是在编译器使用nodejs被广泛诟病的原因。



## 内存相关

参考：https://github.com/overnote/over-javascript/blob/master/12-%E5%89%8D%E7%AB%AF%E4%B9%8B%E7%A6%85/01.2-%E5%86%85%E5%AD%98%E7%AE%A1%E7%90%86-V8%E5%BC%95%E6%93%8E%E4%B8%8E%E5%88%86%E4%BB%A3%E5%9B%9E%E6%94%B6.md
...
{
  rss: 20824064,                // resident set size：所有内存占用，包括指令区、堆栈，表当前进程分配的内存。常驻内存，计算参考：https://www.jianshu.com/p/9bf36aa82f90
  heapTotal: 7159808,           // 堆内存已申请到空间，包括已使用和未使用的
  heapUsed: 4270176,            // 堆内存当前使用量，可以用来判断内存泄露
  external: 8224,                // v8 内部的 C++ 对象占用的内存
  arrayBuffers: 3374650,        // 缓存区大小
}
```

* 从上可以看出，nodejs 主要两种内存：堆内存 和 栈、变量、指令等
...
![v8 内存占用](https://github.com/overnote/over-javascript/raw/master/images/zen/v8-01.svg)
...
* 随着存储对象的增加，进行动态申请
* V8 GC 因增量或者非增量，时长不同
* V8 做出内存限制，本身是由于浏览器不需要很大的内存，导致做出当前 GC 调优限制
...
* app.get 方法默认已经做了 decode，就算前端传入的参数已经进行了 encode，也不用再进行 decode 操作。
...
代码：code -> koa-middleware.ts

建议参考：https://cn.redux.js.org/understanding/history-and-design/middleware#%E5%B0%9D%E8%AF%95-6%E7%AE%80%E5%8D%95%E7%B2%97%E6%9A%B4%E7%9A%84-middleware-%E8%B0%83%E7%94%A8%E6%96%B9%E6%B3%95

看看redux是怎么从 monkey patch 出发，一步一步实现自己的 middleware 的，然后再看看 koa 实现的middleware
...
## 为什么 node 不适合进行cpu密集计算呢？

1. 首先从 js 引擎的垃圾回收限制出发
   ...
   > - 32 位系统下约为 0.7G
   >
   > 在 V8 中，1.4G 以上内存进行一次完全的垃圾回收，需要 1 秒以上。
   >
   ...
   >
   > https://github.com/overnote/over-javascript/blob/master/12-%E5%89%8D%E7%AB%AF%E4%B9%8B%E7%A6%85/01.2-%E5%86%85%E5%AD%98%E7%AE%A1%E7%90%86-V8%E5%BC%95%E6%93%8E%E4%B8%8E%E5%88%86%E4%BB%A3%E5%9B%9E%E6%94%B6.md

  由于js引擎的GC限制，导致js不能够进行大数据计算

2. 从js引擎的编译方式出发

   js 引擎采用的事 JIT，也就是实时编译，相比起 AOT 编译的方式，解释性语言需要一边编译一边执行，再一边垃圾回收，效率不高。
