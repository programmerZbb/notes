...
* 标记这次状态更新为transition（非紧急渲染），在切换tab和输入内容联想搜索的过程中非常有用——原理
...
* 与 useTransition: 这个是一个 hook，不能再条件分支中使用，建议在 react 内部使用
* 与 useDeferredValue 比较：这个是单个值的 transition，建议对 props 或者其他 state 值使用

### 避免不必要的加载指示器

参考：https://zh-hans.react.dev/reference/react/useTransition#preventing-unwanted-loading-indicators

也就是说 suspense 包括的请求，如果不想出现请求过程中出现 fallback 的场景，可以使用 startTransition，让接下来的更新标记为 transition，通过 transition 返回的 loading 状态进行样式变化，而不是直接展示 suspense 的 fallback 组件。

### 不要再输入框里执行 transition

* 要不然会出现UIbug，因为transition的更新会落后于直接输入。

### 和 debounce 关系

有那么一点相似，不过 transition 每次都会执行



## Suspense 和 ErrorBoundary 关系

参考：https://juejin.cn/post/7315231440777527334?searchId=20240423125553201025B550D820A275BF

在源码中都是通过捕获 react 流程来实现的，一个大的 try...catch

* errorBoundary 是通过捕获一般错误实现的
* Suspense 是通过捕获 promise 错误实现的。源码中通过判断该函数的 then 方法是一个函数判断定是 Promise 函数。
...
* 与 lazy 对比：https://loadable-components.com/docs/loadable-vs-react-lazy/
